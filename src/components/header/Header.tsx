...
export default function Header() {
    ...
    useEffect(() => {
        ...
        return () => document.removeEventListener("click", (e) => handleDetailsBackdropClick(e, detailsRef));
    }, []);
        ...
        <header className="w-full h-auto flex flex-row justify-between items-center px-20 p-4 border-b border-gray-300 max-sm:px-6">
            ...
            <div className="flex flex-row justify-between items-center gap-2 font-raleway max-sm:hidden">
                {user.username ? (
                    <details ref={detailsRef}>
                        ...
                        <div className="z-100 w-80 h-36 absolute right-0 flex flex-col rounded-lg border border-gray-400 bg-white shadow-[0px_1px_4px_0px_#00000026]">
                            <div className="flex-1 flex flex-row justify-between items-end px-6 py-8">
                                ...
                                <div onClick={() => onLogout().then(() => handleSetUser({ username: "", email: "" })).catch((error) => toast("error", { header: "Logout Error", message: error.message }))} className="cursor-pointer">
                                    <LogOutIcon />
                                </div>
                            </div>
                    ...
                    <>
                        <PrimaryButton btnType="dark" className="w-25 h-10 rounded-sm" type="button" onClick={() => navigate("/register", { state: { background: isMobile ? location : null } })}>
                            Sign up
                        </PrimaryButton>
                        <PrimaryButton btnType="light" className="w-25 h-10 rounded-sm" type="button" onClick={() => navigate("/login", { state: { background: isMobile ? location : null } })}>
                            Login
                        </PrimaryButton>
                    </>
                }
