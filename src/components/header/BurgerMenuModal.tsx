...
const BurgeMenuModal = forwardRef<HTMLDialogElement>((_props, ref) => {
  ...
  return (
    <dialog
      ref={headerDialogRef}
      onClick={(e) => handleDialogBackdropClick(e, headerDialogRef)}
      className="box-content w-full h-fit px-6 bg-white border-2 border-gray-300 sm:hidden backdrop:backdrop-blur-sm"
    >
      <div className="w-full h-auto flex flex-col justify-start items-start gap-10 p-4 ">
        ...
        <div>
          <Link to="/quizlisting">
            <span className="font-semibold text-lg leading-8 tracking-normal text-gray-600">
              Quizzes
            </span>
          </Link>
        </div>
