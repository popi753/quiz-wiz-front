...
const borderDissapearingTime = 3000; 
...
export default function Toast({ type, toast, icon }: { type: string, toast: Toast, icon: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    }, borderDissapearingTime);
    return () => clearTimeout(timeout);
  }, []);

  return (
    <div className="absolute z-100 top-5 right-5 flex gap-2.5 w-86 max-h-40 text-white ">
      <div className={cn("w-full h-full rounded-lg px-4 py-3 gap-4 bg-gray-700 shadow-[0px_16px_24px_0px_#00000024] flex flex-row items-center p-4 border-b-4 duration-3000!",
        {
      ...
      >
        <div className={cn("bg-gray-600 min-w-8 h-8 flex items-center justify-center rounded-full",
          {
